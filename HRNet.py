...
class HighResolutionModuleBayes(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
                 num_channels, fuse_method, multi_scale_output=True):
        super(HighResolutionModuleBayes, self).__init__()
        self._check_branches(
            num_branches, blocks, num_blocks, num_inchannels, num_channels)
...
class ModuleWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        return x, kl.to(x.device())

class BBBConv2d(ModuleWrapper):

    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding='same', dilation=1, bias=True, priors=None, returnKL=True):
    ...
    def forward(self, x, sample=True):
        ...
        if self.training or sample:
            eps = torch.empty(act_mu.size()).normal_(0, 1).to(x.device)
            if self.returnKL is True:
...
class Aleatoric_net(nn.Module):

    def __init__(self, config, **kwargs):
        global ALIGN_CORNERS
        extra = config["arch"]["EXTRA"]
        super(Aleatoric_net, self).__init__()
        ALIGN_CORNERS = config["arch"]["ALIGN_CORNERS"]

        # noise1 = Normal(torch.zeros((720,1,1)), torch.full((720,1,1),0.1))
        # noise2 = Normal(0.0,0.1)
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        var_out = self.var_layer(x)
        var_out = F.interpolate(input=var_out, size=(h, w), mode='bilinear', align_corners=ALIGN_CORNERS)
...
class HRNet_var(nn.Module):

    def __init__(self, config, **kwargs):
        global ALIGN_CORNERS
        extra = config["arch"]["EXTRA"]
        super(HRNet_var, self).__init__()
        ALIGN_CORNERS = config["arch"]["ALIGN_CORNERS"]
        ...
        self.bayesConv1 = BBBConv2d(
            ...
            out_channels=self.stage2_cfg['NUM_CHANNELS'][0],
            kernel_size=3,
            stride=1,
            padding='same')

        self.bayesConv2 = BBBConv2d(
            in_channels=self.stage2_cfg['NUM_CHANNELS'][-1],
            out_channels=self.stage2_cfg['NUM_CHANNELS'][-1],
            kernel_size=3,
            stride=1,
            padding='same')

        self.bayesConv3 = BBBConv2d(
            in_channels=self.stage3_cfg['NUM_CHANNELS'][-1],
            out_channels=self.stage3_cfg['NUM_CHANNELS'][-1],
            kernel_size=3,
            stride=1,
            padding='same')

        self.bayesConv4 = BBBConv2d(
            in_channels=self.stage4_cfg['NUM_CHANNELS'][-1],
            out_channels=self.stage4_cfg['NUM_CHANNELS'][-1],
            kernel_size=3,
            stride=1,
            padding='same')
    ...
    def _make_stage(self, layer_config, num_inchannels,
                    multi_scale_output=True, bayes=False):
        num_modules = layer_config['NUM_MODULES']
        num_branches = layer_config['NUM_BRANCHES']
        num_blocks = layer_config['NUM_BLOCKS']
        num_channels = layer_config['NUM_CHANNELS']
        ...
        for i in range(num_modules):
            ...
            if bayes is False:
                ...
                modules.append(
                    HighResolutionModuleBayes(num_branches,
                                         block,
                                         num_blocks,
                                         num_inchannels,
                                         num_channels,
    ...
    def forward(self, x):
        ...
        x0, kl0 = self.bayesConv1(x[0])
        x1, kl1 = self.bayesConv2(x[1])
        x2, kl2 = self.bayesConv3(x[2])
        x3, kl3 = self.bayesConv4(x[3])
